...
with col_search:
    keyword = st.text_input("🔍 Cari Site ID / Nama Site / kata kunci lain (opsional):", placeholder="Contoh: R1200302 / BANGUN PURBA / JAILOLO")

# Jika ada keyword pencarian, filter dataframe-nya agar tabel tidak kepanjangan
if keyword:
    # Mencari kata kunci di seluruh sel/kolom dalam DataFrame
...
if tombol_simpan:
    try:
        with st.spinner("Menyimpan perubahan ke Google Sheets..."):
            # Jika sedang difilter, kita update hanya baris yang diedit ke dataframe asli
            if keyword:
                df.update(edited_df)
                data_to_save = df
            else:
